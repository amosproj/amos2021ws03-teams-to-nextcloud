...
class Delete extends Action {
    ...
    async execute () {
        ...
        this.emitter.emit("SHOW_PROGRESS_BAR", "Deleting " + selected.length + " item" + (selected.length == 1 ? "" : "s") + "...");
        ...
        for (let i = 0; i < selected.length; i++) {
            ...
            this.emitter.emit("PROGRESS_BAR_WIDTH", (deletedFiles / selected.length) * 100);
            
            // Hide the progress bar when all the selected files have been deleted
            if (deletedFiles == selected.length) {
                setTimeout(() => this.emitter.emit("HIDE_PROGRESS_BAR"), 1000);
            }
            
            // Show a notification
            this.toast.success("Deleted " + currentFile.name);
        }
